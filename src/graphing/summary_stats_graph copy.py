...
def create_bar_chart() -> None:
    ...
    # Create bars for faithfulness subplot (left)
    faithfulness_bars = []
    for cat_idx, category_center in enumerate(category_centers):
        ...
        for model_idx in range(3):
            ...
            bar_height = bar_heights[cat_idx][model_idx]
        ...
        faithfulness_bars.append(category_bars)
    
    # Create bars for detection subplot (right)
    detection_bars = []
    for cat_idx, category_center in enumerate(category_centers):
        category_bars = []
        for model_idx in range(3):
            # Calculate position for this bar within the category
            bar_x = category_center - bar_spacing + model_idx * bar_spacing
            bar_height = bar_heights[cat_idx + 2][model_idx]  # +2 for detection categories
            bar_color = model_colors[model_idx]
        ...
        detection_bars.append(category_bars)
    ...
    ax1.set_xticklabels(categories[:2], fontsize=11, linespacing=1.8)  # Faithfulness categories
    
    ax2.set_xticks(category_centers)
    ax2.set_xticklabels(categories[2:], fontsize=11, linespacing=1.8)  # Detection categories
